...
class Vehiculo {
    ...
    public function obtenerVehiculos($filtros = []) {
        try {
            $sql = "SELECT v.*, c.nombre as cliente_nombre, c.apellido as cliente_apellido 
                    ...
                    LEFT JOIN " . $this->table_clientes . " c ON v.codigo_cliente = c.codigo 
                    WHERE 1=1";
    ...
    public function editarVehiculo($id, $datos) {
        try {
            ...
            if (isset($datos['placa'])) {
                $sqlCheck = "SELECT id FROM " . $this->table_vehiculos . " WHERE placa = ? AND id != ?";
                $stmtCheck = $this->conn->prepare($sqlCheck);
                $stmtCheck->bind_param("si", $datos['placa'], $id);
                $stmtCheck->execute();
            ...
            $sql = "UPDATE " . $this->table_vehiculos . " SET " . implode(', ', $campos) . " WHERE id = ?";
            $valores[] = $id;
            $types .= "i";
    ...
    public function eliminarVehiculo($id) {
        try {
            ...
            $sql = "SELECT en_estacionamiento FROM " . $this->table_vehiculos . " WHERE id = ?";
            $stmt = $this->conn->prepare($sql);
            $stmt->bind_param("i", $id);
            $stmt->execute();
            $result = $stmt->get_result();
            ...
            $sql = "DELETE FROM " . $this->table_vehiculos . " WHERE id = ?";
            $stmt = $this->conn->prepare($sql);
            $stmt->bind_param("i", $id);
            $resultado = $stmt->execute();
    ...
    private function placaExiste($placa) {
        $query = "SELECT id FROM " . $this->table_vehiculos . " WHERE placa = ?";
        $stmt = $this->conn->prepare($query);
    ...
    private function obtenerClientePorCodigo($codigo) {
        $query = "SELECT id FROM " . $this->table_clientes . " WHERE codigo = ?";
        $stmt = $this->conn->prepare($query);
    ...
    public function crearCliente($datos) {
        try {
            ...
            $query = "INSERT INTO " . $this->table_clientes . " 
                      (codigo, nombre, apellido, telefono, email, direccion) 
                      VALUES (?, ?, ?, ?, ?, ?)";
